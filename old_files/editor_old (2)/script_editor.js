document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener('mouseup', (event) => {
        ...
        if (selectedText.rangeCount > 0 && !selectedText.isCollapsed) {
            ...
            if (selectedElement.tagName === 'SPAN' && selectedElement.hasAttribute('class') && selectedElement.getAttribute('class').includes('data-index')) {
                indexInput.value = selectedElement.getAttribute('class');
            } else {
                indexInput.value = '';
            }
        } else {
    ...
    setIndexButton.addEventListener('click', () => {
        if (selectedRange) {
            if (selectedElement && selectedElement.tagName === 'SPAN' && selectedElement.hasAttribute('data-index')) {
                ...
                selectedElement.setAttribute('class', 'data-index_' + indexInput.value);
            } else {
                // Create new span
                const selectedContent = selectedRange.extractContents();
                const span = document.createElement('span');
                span.appendChild(selectedContent);
                span.setAttribute('class', 'data-index_' + indexInput.value);
